...
def queue_management():
    ...
    if now_serving:
        ...
        now_serving['room'] = f"Room {(q_num % 5) + 1}"
...
def analytics2():
    ...
    # 3. Clinic Performance (Mocked based on doctor association if we had it, 
    # but we'll just group by doctor for now or mock clinics)
    # Let's fetch clinics to show them
    clinics = conn.execute('SELECT name FROM clinics').fetchall()
    clinic_names = [row['name'] for row in clinics]
    # Mock clinic ratings
    clinic_ratings = [round(random.uniform(4.0, 5.0), 1) for _ in clinic_names]
...
def patient_queue_status():
    ...
    # Count people ahead: Same doctor, same date, active status, earlier time
    ...
    print(f"DEBUG: {ahead_count} people ahead")
